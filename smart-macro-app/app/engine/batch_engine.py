...
ACTION_LABELS = {
    ...
    "smart_process":     "⚙️  Smart Process (Custom AI Instruction)",
...
def run_batch(
    ...
    def _worker():
        ...
        if not files:
            thought_cb("⚠️  No supported files found in the selected folder.\n")
...
def _dispatch(filepath, ext, action, instruction, options, style, thought_cb):
    ...
    if action == "enhance_docs":
        if ext in WORD_EXTS:
            ...
            thought_cb(f"⏭️  Skipping unsupported type for enhance: {ext}\n")
            return None
    ...
    # ── FORMAT EXCEL ────────────────────────────────────────────────────────
        ...
        if ext in EXCEL_EXTS:
            ...
            thought_cb(f"⏭️  Skipping non-Excel file: {os.path.basename(filepath)}\n")
            return "skipped"
    ...
    # ── SMART PROCESS ───────────────────────────────────────────────────────
        ...
        if ext in WORD_EXTS:
            ...
            thought_cb(f"⏭️  Smart Process supports .docx/.xlsx only — skipping {ext}\n")
            return "skipped"
    ...
    # ── GENERATE SUMMARY ────────────────────────────────────────────────────
        ...
        thought_cb(f"⚠️  Unknown action: {action}\n")
        return "skipped"
...
def _enrich_pptx(filepath, opts, style, thought_cb):
    ...
    - Fix spelling / grammar in text boxes
